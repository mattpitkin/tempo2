	  ...
	  for (ii=0;ii<nglobal;ii++)
	    {
	      ...
	      iglobal = psr[0].fitParamI[ii];
	      kglobal = psr[0].fitParamK[ii];
	      fprintf(stderr, "%d %d %d %d %d\n", ii,iglobal,kglobal,param_quad_ifunc_p, param_quad_ifunc_c);
	      ...
	      if ((iglobal == param_quad_ifunc_p) || (iglobal == param_quad_ifunc_c))
		{
		  ...
		  for(jj=0;jj<nglobal;jj++)
		    {
		      pglobal =  psr[0].fitParamI[jj];
		      qglobal = psr[0].fitParamK[jj];
		      ...
		      if ((pglobal == param_quad_ifunc_p) || (pglobal  == param_quad_ifunc_c))
			{
			  ...
			  if(paramout) fprintf(paramout, "%d %d %.3e \n", ii,jj, cvm[ii][jj]);
...
int getNglobal(pulsar *psr,int npsr){
   ...
   // adding in fitting for sinusoidal dm
...
double getParamDeriv(pulsar *psr,int ipos,double x,int i,int k)
{
   ...
   else if (i==param_wave_om) /* Whitening procedure using sinusoids */
   {
	  ...
	  if (psr->waveScale==0)
	  {
	    if (floor(k/2.0) ==0)
	      {
		om=  psr->param[param_wave_om].val[0]*1.0;
	      }
	    else
	      {
		om= psr->param[param_wave_om].val[0]* (floor (k/2.0)+1.0);
	      }
	    
	    if (k%2==0) afunc = cos(om*(x-Xoff)); 
	    else        afunc = sin(om*(x-Xoff)); 
	  ...
	  else if (psr->waveScale==2)
	  {
		 ...
		 if (k < psr->nWhite*2)
		 {
		   ...
		   if (k%2==0) 
		     {
		       //afunc = cos(om*(floor((k-(psr->nWhite*2))/2.0)+1)*x)/(DM_CONST*freq*freq);
		       afunc = cos(om*(floor(k/2.0)+1)*(x-Xoff))/(DM_CONST*freq*freq);
		     }
		   else
		     {
		       //afunc = sin(om*(floor((k-(psr->nWhite*2))/2.0)+1)*x)/(DM_CONST*freq*freq);
		       afunc = sin(om*(floor(k/2.0)+1)*(x-Xoff))/(DM_CONST*freq*freq);
		     }
		 }
		 else
		 {
		   
		   if ((k-(psr->nWhite*2))%2==0) afunc = cos(om*(floor((k-(psr->nWhite*2))/2.0)+1)*(x-Xoff));
		   else afunc = sin(om*(floor((k-(psr->nWhite*2))/2.0)+1)*(x-Xoff));
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	for(int i=0;i<FitRedCoeff/2;i++)
	  {
	    ...
	    pulse->TNRedcos[i] = maxcoeff[numtofit+i];
	    pulse->TNRedcos_err[i] =Errorvec[numtofit+i];
	  }
 
	for(int i=0;i<FitRedCoeff/2;i++)
	  {
	    pulse->TNRedsin[i] = maxcoeff[numtofit+i+FitRedCoeff/2];  
	    pulse->TNRedsin_err[i] = Errorvec[numtofit+i+FitRedCoeff/2];
	  }
	...
	for(int i=0;i<FitRedCoeff/2;i++)
	  {
	    float pow, pow_err;
	    pow= powf( pulse->TNRedsin[i], 2.) +   powf( pulse->TNRedcos[i], 2.);
	    pow_err = sqrt(powf( 2*pulse->TNRedsin[i]*pulse->TNRedsin_err[i] , 2.) +   powf( 2*pulse->TNRedcos[i]* pulse->TNRedcos_err[i]  , 2.));

	    fprintf(stdout,"FREQ %.3e %.3e %.3e %.3e %.3e %.3e %.3e\n", freqs[i], pulse->TNRedsin[i],  pulse->TNRedsin_err[i],  pulse->TNRedcos[i], pulse->TNRedcos_err[i], pow, pow_err );
	  }
	...
	fprintf(stderr, "exited new dofit");
	...
	*/
	...
	for(int i=0;i<numtofit; i++){
		...
		if(useOrthogonal==0){
			...
			TempoErr[i]=Errorvec[i]/TNDMScale[i];
		}
	}
	updateParameters(pulse,0,TempoCoeff,TempoErr);
	...
	for(int i=0;i<pulse->nobs;i++){
		...
		for(int j=0;j<totalsize; j++){
			...
			if(j>=FitRedCoeff+numtofit && j < numtofit+totCoeff+DMEventQuadTerms+ShapeEventTerms){
				dmsum+=TotalMatrix[i][j]*maxcoeff[j];
				dmerr+=pow(TotalMatrix[i][j]*Errorvec[j],2);
			}
		}
