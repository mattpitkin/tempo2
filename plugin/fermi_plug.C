		...
		for (i=1;i<nrows2;i++)
		{
	        ...
	        if (ophase == 0)
                ...
                else
                {
                    ...
                    // If ELL1 model with FBx parameters
                    else if (strcmp(psr[0].binaryModel,"ELL1")==0 && psr[0].param[param_fb].paramSet[0]) 
                    {
                        tt0 = (psr[0].obsn[i].bat-psr[0].param[param_tasc].val[0])*SECDAY;
                        tpb = psr[0].param[param_fb].val[0]*tt0;
                        fac = 1.0;
                    ...
                    // If PB and T0 are set (note that PBDOT is used in the calculation)
