...
#include <math.h>
#include "tempo2.h"
#include "GWgeneralsim_anisotropic.h"
#include "T2toolkit.h"
#include "TKfit.h"
#include <cpgplot.h>
...
void help() /* Display help */
{
  ...
  printf("-harmfileTT input file containing list of harmonics for transverse tensor background - l m amp - to include in background.\n"); //poltype is TT (0), ST (1), SL (2) or VL (3)\n");
  printf("-harmfileST input file containing list of harmonics for transverse scalar background\n");
  printf("-harmfileSL input file containing list of harmonics for longitudinal scalar background\n");
  printf("-harmfileVL input file containing list of harmonics for longitudinal vector background\n");
  ...
  printf("\n\n");
  printf("Example usage: tempo2 -gr GWgeneralanisobkgrd -f tt.par 0437.2048.tim -dist 1 -gwampTT 1e-15 -alphaTT -0.666666 -harmfileTT TTBkgrdHarmonics -gwampST 1e-15 -alphaST -0.666666 -harmfileST STBkgrdHarmonics -gwampSL 1e-15 -alphaSL -0.666666 -harmfileSL SLBkgrdHarmonics -gwampVL 1e-15 -alphaVL -0.66666 -harmfileVL VLBkgrdHarmonics -ngw 1000 -plot\n\n");
  printf("Plot options\n\n");
...
extern "C" int graphicalInterface(int argc,char *argv[],pulsar *psr,int *npsr) 
{
  ...
  *npsr = 0;  /* For a graphical interface that only shows results for one pulsar */
  ...
  printf("Graphical Interface: GWanisobkgrd\n");
  printf("Author:              J. Gair, modified from GWbkgrd by G. Hobbs\n");
  printf("Version:             1.0\n");
  printf(" --- type 'h' for help information\n");

  harmlist=(double ***)malloc(4*sizeof(double **));
  ...
  for (i=2;i<argc;i++)
      ...
      else if (strcmp(argv[i],"-dist")==0) // Distance in kpc
	{
      ...
      else if (strcmp(argv[i],"-harmfileTT")==0)
	{
	  strcpy(harmFile,argv[i+1]);
	  setharmfileTT=1;
	  harmin=fopen(harmFile,"r");
	  fscanf(harmin,"%i",&(nharms[0]));
	  //printf("%i\n",nharms);
	  harmlist[0]=(double **)malloc(nharms[0]*sizeof(double *));
	  for (int hi=0;hi<nharms[0];hi++) {
		harmlist[0][hi]=(double *)malloc(3*sizeof(double));
		for (int hj=0;hj<3;hj++) {
			fscanf(harmin,"%lf",&(harmlist[0][hi][hj]));
			//printf("%6.4e\n",harmlist[hi][hj]);
		}
	  }
	  fclose(harmin);
	}
      else if (strcmp(argv[i],"-harmfileST")==0)
	{
	  strcpy(harmFile,argv[i+1]);
	  setharmfileST=1;
	  harmin=fopen(harmFile,"r");
	  fscanf(harmin,"%i",&(nharms[1]));
	  //printf("%i\n",nharms);
	  harmlist[1]=(double **)malloc(nharms[1]*sizeof(double *));
	  for (int hi=0;hi<nharms[1];hi++) {
		harmlist[1][hi]=(double *)malloc(3*sizeof(double));
		for (int hj=0;hj<3;hj++) {
			fscanf(harmin,"%lf",&(harmlist[1][hi][hj]));
			//printf("%6.4e\n",harmlist[hi][hj]);
		}
	  }
	  fclose(harmin);
	}
      else if (strcmp(argv[i],"-harmfileSL")==0)
	{
	  strcpy(harmFile,argv[i+1]);
	  setharmfileSL=1;
	  harmin=fopen(harmFile,"r");
	  fscanf(harmin,"%i",&(nharms[2]));
	  //printf("%i\n",nharms);
	  harmlist[2]=(double **)malloc(nharms[2]*sizeof(double *));
	  for (int hi=0;hi<nharms[2];hi++) {
		harmlist[2][hi]=(double *)malloc(3*sizeof(double));
		for (int hj=0;hj<3;hj++) {
			fscanf(harmin,"%lf",&(harmlist[2][hi][hj]));
			//printf("%6.4e\n",harmlist[hi][hj]);
		}
	  }
	  fclose(harmin);
	}
      else if (strcmp(argv[i],"-harmfileVL")==0)
	{
	  strcpy(harmFile,argv[i+1]);
	  setharmfileVL=1;
	  harmin=fopen(harmFile,"r");
	  fscanf(harmin,"%i",&(nharms[3]));
	  //printf("%i\n",nharms);
	  harmlist[3]=(double **)malloc(nharms[3]*sizeof(double *));
	  for (int hi=0;hi<nharms[3];hi++) {
		harmlist[3][hi]=(double *)malloc(3*sizeof(double));
		for (int hj=0;hj<3;hj++) {
			fscanf(harmin,"%lf",&(harmlist[3][hi][hj]));
			//printf("%6.4e\n",harmlist[hi][hj]);
		}
	  }
	  fclose(harmin);
  ...
  if ((setharmfileTT==0) && (gwAmps.tensor_amp))
    {
      nharms[0]=1;
      harmlist[0]=(double **)malloc(sizeof(double *));
      harmlist[0][0]=(double *)malloc(3*sizeof(double));
      harmlist[0][0][0]=harmlist[0][0][1]=0.;
      harmlist[0][0][2]=1.;
      printf("WARNING: no TT harmonic file found, but amplitude specified. Generating an isotropic background.\n");
    }

  if ((setharmfileST==0) && (setgwAmpST))
    {
      nharms[1]=1;
      harmlist[1]=(double **)malloc(sizeof(double *));
      harmlist[1][0]=(double *)malloc(3*sizeof(double));
      harmlist[1][0][0]=harmlist[1][0][1]=0.;
      harmlist[1][0][2]=1.;
      printf("WARNING: no ST harmonic file found, but amplitude specified. Generating an isotropic background.\n");
    }

  if ((setharmfileSL==0) && (setgwAmpSL))
    {
      nharms[2]=1;
      harmlist[2]=(double **)malloc(sizeof(double *));
      harmlist[2][0]=(double *)malloc(3*sizeof(double));
      harmlist[2][0][0]=harmlist[2][0][1]=0.;
      harmlist[2][0][2]=1.;
      printf("WARNING: no SL harmonic file found, but amplitude specified. Generating an isotropic background.\n");
    }

  if ((setharmfileVL==0) && (setgwAmpVL))
    {
      nharms[3]=1;
      harmlist[3]=(double **)malloc(sizeof(double *));
      harmlist[3][0]=(double *)malloc(3*sizeof(double));
      harmlist[3][0][0]=harmlist[3][0][1]=0.;
      harmlist[3][0][2]=1.;
      printf("WARNING: no VL harmonic file found, but amplitude specified. Generating an isotropic background.\n");
    }
  ...
  GWgeneralanisotropicbackground(gw,numsources,&seed,flo,fhi,gwAmps,logspacing,harmlist,nharms);
  for (int hj=0;hj<4;hj++) {
  	for (int hi=0;hi<nharms[hj];hi++)
		free(harmlist[hj][hi]);
	free(harmlist[hj]);
  }
  free(harmlist);
