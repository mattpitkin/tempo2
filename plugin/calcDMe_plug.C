// Copyright (C) 2009 Stefan Oslowski, George Hobbs
...
void help() /* Display help */ {
    ...
    printf("   DEFAULT: smoothing_width=100days\n");
...
extern "C" int graphicalInterface(int argc, char *argv[], pulsar *psr, int *npsr) {
    ...
    gr = (char*) malloc(100);
    xlab = (char*) malloc(100);
    ylab = (char*) malloc(100);
...
void callFit(pulsar *psr, int npsr) {
    ...
    for (it = 0; it < psr[0].nits; it++) {
        ...
        for (iteration = 0; iteration < 2; iteration++) {
            ...
            } else if (debugFlag >= 1) textOutput(psr, npsr, globalParameter, 0, 0, 1, "fit.par");
        }
    }
    if (psr[0].nFit == 0) {
