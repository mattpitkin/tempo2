...
/* This plugin allows the user to simulate an anisotropic gravitational wave background */
...
void help() /* Display help */
{
  ...
  printf("-harmfile input file containing list of harmonics - l m amp - to include in background\n");
  ...
  printf("\n\n");
  printf("Example usage: tempo2 -gr GWanisobkgrd -f tt.par 0437.2048.tim -dist 1 -gwamp 1e-15 -alpha -0.666666 -harmfile BkgrdHarmonics -ngw 1000 -plot\n\n");
  printf("Plot options\n\n");
  ...
  *npsr = 0;  /* For a graphical interface that only shows results for one pulsar */
  ...
  for (i=2;i<argc;i++)
      ...
      else if (strcmp(argv[i],"-harmfile")==0)
	{
	  ...
	  fscanf(harmin,"%i",&nharms);
	  //printf("%i\n",nharms);
	  harmlist=(double **)malloc(nharms*sizeof(double *));
	  for (int hi=0;hi<nharms;hi++) {
		harmlist[hi]=(double *)malloc(3*sizeof(double));
		for (int hj=0;hj<3;hj++) {
			fscanf(harmin,"%lf",&(harmlist[hi][hj]));
			//printf("%6.4e\n",harmlist[hi][hj]);
		}
	  }
	  fclose(harmin);
  ...
  if (setharmfile==0)
    {
      nharms=1;
      harmlist=(double **)malloc(sizeof(double *));
      harmlist[0]=(double *)malloc(3*sizeof(double));
      harmlist[0][0]=harmlist[0][1]=0.;
      harmlist[0][2]=1.;
      printf("WARNING: no harmonic file found, generating an isotropic background.\n");
    }
  ...
  GWanisotropicbackground(gw,ngw,&seed,flo,fhi,gwAmp,alpha,logspacing,harmlist,nharms);
  for (int hi=0;hi<nharms;hi++)
	free(harmlist[hi]);
  free(harmlist);
