...
void drawOption(int on,float x,float y,const char *str);
...
extern "C" int graphicalInterface(int argc,char *argv[],pulsar *psr,int *npsr) 
{
    ...
    *npsr = 0;  /* For a graphical interface that only shows results for one pulsar */
    ...
    if (res==1) // Replace residuals with residual file
    {
        ...
        while (!feof(fin))
        {
            if (fscanf(fin,"%s %s %lf",s1,s2,
                        &psr[0].obsn[psr[0].nobs].toaErr)==3){

                psr[0].obsn[psr[0].nobs].sat = parse_longdouble(s1);
                psr[0].obsn[psr[0].nobs].residual = parse_longdouble(s2);
                psr[0].nobs++;
            }
        }
        fclose(fin);
