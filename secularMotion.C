...
void secularMotion(pulsar *psr,int npsr)
{
    ...
    for (p=0;p<npsr;p++)
    {
        for (i=0;i<psr[p].nobs;i++)	
        {
            ...
            {
                t0 = (psr[p].obsn[i].bat - psr[p].param[param_posepoch].val[0])*longdouble(86400.0);	      
                psr[p].obsn[i].shklovskii = t0*t0/longdouble(2.0)/longdouble(299792458.0)*
            ...
            psr[p].obsn[i].bbat = psr[p].obsn[i].bat - (psr[p].obsn[i].shklovskii)/longdouble(86400.0);
        }
    }
}
