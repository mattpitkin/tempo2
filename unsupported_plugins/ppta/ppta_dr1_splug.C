...
extern "C" int selectInterface(int argc,char *argv[],pulsar *psr,int *npsr) 
{
  ...
  int nsimul;
  int remove50=0,remove20=0,remove10=0;
  ...
  for (i=0;i<argc;i++)
	...
	sscanf(argv[++i],"%s",selectFname);
      else if (strcmp(argv[i],"-remove50")==0)
	remove50=1;
      else if (strcmp(argv[i],"-remove20")==0)
	remove20=1;
      else if (strcmp(argv[i],"-remove10")==0)
	remove10=1;
    }
  ...
  for (p=0;p<*npsr;p++)
      ...
      for (i=0;i<psr[p].nobs;i++)
	{
	  ...
	  // Now check whether the observation is within a suitable time range
	  if (psr[p].obsn[i].sat <= 53430 || psr[p].obsn[i].sat > 55621)
	    psr[p].obsn[i].deleted = 1;
	  if (remove10==1 && psr[p].obsn[i].freq > 2500 && psr[p].obsn[i].freq < 4000)
	    psr[p].obsn[i].deleted = 1;
	  if (remove20==1 && psr[p].obsn[i].freq > 1000 && psr[p].obsn[i].freq < 2500)
	    psr[p].obsn[i].deleted = 1;
	  if (remove50==1 && psr[p].obsn[i].freq > 500 && psr[p].obsn[i].freq < 1000)
	    psr[p].obsn[i].deleted = 1;

	  // Check for unusual observing modes
	  for (k=0;k<psr[p].obsn[i].nFlags;k++)
	    {
	      if (strcmp(psr[p].obsn[i].flagID[k],"-f")==0 &&
		  strcmp(psr[p].obsn[i].flagVal[k],"10CM_PDFB3")==0)
		psr[p].obsn[i].deleted=1;
	      if (strcmp(psr[p].obsn[i].flagID[k],"-f")==0 &&
		  strcmp(psr[p].obsn[i].flagVal[k],"10CM_CPSR2")==0)
		psr[p].obsn[i].deleted=1;
	    }
